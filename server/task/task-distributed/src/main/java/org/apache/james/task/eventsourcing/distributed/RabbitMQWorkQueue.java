...
public class RabbitMQWorkQueue implements WorkQueue, Startable {
    ...
    private Mono<Task.Result> executeTask(AcknowledgableDelivery delivery) {
        ...
        try {
            ...
            LOGGER.error("Unable to deserialize submitted Task " + taskId.asString(), e);
            worker.fail(taskId, Task.UNKNOWN, Optional.empty(), e);
            return Mono.empty();
        } finally {
            delivery.ack();
        }
        ...
        try {
            ...
            worker.fail(taskId, task.type(), task.details(), e);
            return Mono.empty();
        }
    }
