...
class SerialTaskManagerWorkerTest {
    ...
    void aSuccessfullTaskShouldCompleteSuccessfully() {
        ...
        verify(listener, atLeastOnce()).completed(taskWithId.getId(), Task.Result.COMPLETED, CompletedTask.TYPE, Optional.empty());
    }
    ...
    void aFailedTaskShouldCompleteWithFailedStatus() {
        ...
        verify(listener, atLeastOnce()).failed(taskWithId.getId(), FailedTask.TYPE, Optional.empty());
    }
    ...
    void aThrowingTaskShouldCompleteWithFailedStatus() {
        ...
        assertThat(result.block()).isEqualTo(Task.Result.PARTIAL);
        verify(listener, atLeastOnce()).failed(eq(taskWithId.getId()), eq(ThrowingTask.TYPE), eq(Optional.empty()), any(RuntimeException.class));
    }
    ...
    void theWorkerShouldCancelAnInProgressTask() throws InterruptedException {
        ...
        verify(listener, atLeastOnce()).cancelled(id, MemoryReferenceTask.TYPE, Optional.empty());
        verifyNoMoreInteractions(listener);
    }
