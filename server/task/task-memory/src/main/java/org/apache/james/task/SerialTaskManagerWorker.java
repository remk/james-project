...
public class SerialTaskManagerWorker implements TaskManagerWorker {
    ...
    private Callable<Semaphore> acquireSemaphore(TaskWithId taskWithId, Listener listener) {
        return () -> {
                try {
                    ...
                    listener.cancelled(taskWithId.getId(), taskWithId.getTask().type(), taskWithId.getTask().details());
                    throw e;
                }
            };
    }
                ...
                return Mono.fromFuture(future)
                        .doOnError(exception -> {
                            if (exception instanceof CancellationException) {
                                ...
                                listener.failed(taskWithId.getId(), taskWithId.getTask().type(), taskWithId.getTask().details(), exception);
                            }
                        })
    ...
    private Task.Result run(TaskWithId taskWithId, Listener listener) {
        ...
        try {
            return taskWithId.getTask()
                .run()
                .onComplete(result -> listener.completed(taskWithId.getId(), result, taskWithId.getTask().type(), taskWithId.getTask().details()))
                .onFailure(() -> {
                    LOGGER.error("Task was partially performed. Check logs for more details. Taskid : " + taskWithId.getId());
                    listener.failed(taskWithId.getId(), taskWithId.getTask().type(), taskWithId.getTask().details());
                });
        } catch (InterruptedException e) {
            listener.cancelled(taskWithId.getId(), taskWithId.getTask().type(), taskWithId.getTask().details());
            ...
            listener.failed(taskWithId.getId(), taskWithId.getTask().type(), taskWithId.getTask().details(), e);
            return Task.Result.PARTIAL;
        }
    }
    ...
    public void fail(TaskId taskId, TaskType taskType, Optional<TaskExecutionDetails.AdditionalInformation> additionalInformation, Throwable reason) {
        listener.failed(taskId, taskType, additionalInformation, reason);
    }
