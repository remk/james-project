...
public class MemoryTaskManager implements TaskManager {
    ...
    private static class DetailsUpdater implements TaskManagerWorker.Listener {
        ...
        @Override
        public void completed(TaskId taskId, Task.Result result, TaskType taskType, Optional<TaskExecutionDetails.AdditionalInformation> additionalInformation) {
            updaterFactory.apply(taskId)
                .accept(details -> details.completed(additionalInformation));
        }

        @Override
        public void failed(TaskId taskId, TaskType taskType, Optional<TaskExecutionDetails.AdditionalInformation> additionalInformation, Throwable t) {
            failed(taskId, taskType, additionalInformation);
        }

        @Override
        public void failed(TaskId taskId, TaskType taskType, Optional<TaskExecutionDetails.AdditionalInformation> additionalInformation) {
            updaterFactory.apply(taskId)
                .accept(details -> details.failed(additionalInformation));
        }

        @Override
        public void cancelled(TaskId taskId, TaskType taskType, Optional<TaskExecutionDetails.AdditionalInformation> additionalInformation) {
            updaterFactory.apply(taskId)
